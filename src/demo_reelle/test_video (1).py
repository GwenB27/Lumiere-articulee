...
while True:
    ...
    hauteur_image = 480 #en pixels
    largeur_image = 644 #en pixels

    pixel_central = [largeur_image//2, hauteur_image//2]
    print("pixel central = ", pixel_central)

    phi_vertical = 31.10
    phi_horizontal= 41.08
        ...
        for i, point_set in enumerate(points):
            ...
            if len(point_set) >= 2:
                ...
                if i == 0 :
                    ...
                    L_QR = 6 #longueur du QR code dans la vie réelle en cm
                    L_pixels_QR = abs(point_set[0][0] - point_set[1][0])
                    print("L_pixels_QR: ", L_pixels_QR)

                    theta = phi_vertical * L_pixels_QR / hauteur_image

                    R = L_QR / theta

                    lambda_vertical = x_bleu * phi_vertical / hauteur_image
                    lambda_horizontal = y_bleu * phi_horizontal / largeur_image

                    y = R * np.cos(lambda_vertical * 2 * np.pi / 360)
                    z = R * np.sin(lambda_vertical * 2 * np.pi / 360)
                    x = R * np.sin(lambda_horizontal * 2 * np.pi/360)
